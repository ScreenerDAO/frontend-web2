...
import { IElement, IElementsGroup, balanceSheetLabels, balanceSheetStructure, balanceSheetTypesNames, getFinancialStatementStructureFromCustomLabels, getFinancialStatementStructureFromCustomLabelsV2 } from 'src/types/FinancialStatementsTypes';
        ...
        <>
            {
                calculatedBalanceSheetStructure.map((row, rowIndex) => {
                    if (props.data.CustomLabels?.[row]?.title) {
                        return ( 
                            <Row label={Number(row)} bold={true} key={rowIndex} />
                        )
                    }
                    else {
                        return ( 
                            <Row label={Number(row)} key={rowIndex} />
                        )
                    }
                })
            }
